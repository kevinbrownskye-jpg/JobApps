...
class LoggerAPI:
    ...
    def save_to_local_json_file(self, fresh_jobs_json):
        """
        Receives a batch of jobs from the JS frontend and appends them to
        export_history.json in the app folder, with a proper UTC timestamp
        added to each record for traceability.
        """
        file_path = os.path.join(exe_dir(), "export_history.json")
        try:
            # 1. Parse incoming jobs from the frontend
            new_jobs = json.loads(fresh_jobs_json)
            if not isinstance(new_jobs, list):
                new_jobs = [new_jobs]

            # 2. Load existing history (start fresh if missing or corrupt)
            existing_history = []
            if os.path.exists(file_path):
                try:
                    ...
                    existing_history = []  # corrupt file — start fresh

            # 3. Stamp each job with a UTC timestamp and append
            archived_at = datetime.datetime.utcnow().isoformat() + "Z"
            for job in new_jobs:
                job["offline_archived_at"] = archived_at
                existing_history.append(job)

            # 4. Write the updated list back to disk
...
def main() -> None:
    ...
    # PYWEBVIEW_DEBUG env var allows build_exe.py to bake in debug mode
    # without needing CLI args at runtime
